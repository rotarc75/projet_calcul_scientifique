...
\begin{remark*}
    ...
    Les fonctions des questions 5, 7, 9, 11, 22, 28(b) et 28(c) sont incluses dans le code fourni en annexe.
...
\begin{tikzpicture}[scale=1.5]
    % Tracer le rectangle principal (la grille)
    \draw[thick] (0,0) rectangle (5,3);

    % Ajouter les étiquettes aux coins
    \node[below left] at (0,0) {$0$};
    \node[below right] at (5,0) {$N$};
    \node[above left] at (0,3) {$M$};

    % Tracer les lignes horizontales (en rouge comme sur le brouillon)
    \draw[red] (0, 0.6) -- (5, 0.6);
    \draw[red] (0, 1.2) -- (5, 1.2);
    \draw[red] (0, 1.8) -- (5, 1.8);
    \draw[red] (0, 2.4) -- (5, 2.4);

    % Ajouter les repères 'l' et 'k' (en bleu-vert)
...
\begin{center}
\begin{tikzpicture}[scale=1.5]
    % Tracer la grille principale (4x3 cellules)
    \draw[step=1cm, thick] (0,0) grid (4,3);

    % Tracer les diagonales et placer les labels avec des boucles
    \foreach \y in {0,1,2} {
        \foreach \x in {0,1,2,3} {
            % Calcul du numéro de base pour la cellule (x,y)
            \pgfmathtruncatemacro{\cellnum}{\y*4 + \x}
            \pgfmathtruncatemacro{\tbase}{\cellnum * 2}
            \pgfmathtruncatemacro{\tplus}{\tbase + 1}
            
            % Alternance des diagonales selon la parité de (x+y)
            \pgfmathtruncatemacro{\parity}{mod(\x+\y,2)}
            
            \ifnum\parity=0
                % Diagonale de type "/" (cas pair, ex: première case 0,0)
                \draw (\x, \y) -- (\x+1, \y+1);
                % Placement des labels : T_base en haut à gauche, T_plus en bas à droite
                \node at (\x+0.3, \y+0.75) {\footnotesize $T_{\tbase}$};
                \node at (\x+0.7, \y+0.25) {\footnotesize $T_{\tplus}$};
            \else
                % Diagonale de type "\" (cas impair, ex: deuxième case 1,0)
                \draw (\x, \y+1) -- (\x+1, \y);
                % Placement des labels : T_base en bas à gauche, T_plus en haut à droite
...
\begin{center}
\begin{tikzpicture}[scale=3] % Échelle agrandie pour que le schéma soit bien lisible
    % Tracer les axes
    \draw[->, gray] (-0.3, 0) -- (1.5, 0) node[right, black] {$x$};
    \draw[->, gray] (0, -0.3) -- (0, 1.5) node[above, black] {$y$};

    % Tracer le triangle de référence
    \draw[thick, black] (0,0) -- (1,0) -- (0,1) -- cycle;

    % Placer les sommets et leurs étiquettes
    ...
    % Placer le symbole du triangle à l'intérieur
